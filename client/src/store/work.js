...
const work = {
    ...
    getters: {
        ...
        activeList: (state) => {
            if (state.tags.reduce((acc, cur) => acc + (cur.isActive ? 1 : 0), 0) > 0) {
                ...
                return state.list.filter((item) => item.content.tags.some((tag) => tags.includes(tag)));
            }
            return state.list;
        },
    },
    mutations: {
        ...
        setTags(state) {
            state.tags = Array.from(new Set(state.list.map((item) => item.content.tags).flat())).map(
                (item) => ({ name: item, isActive: false }),
            );
        },
